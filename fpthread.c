...
static inline long steal_work(fp_for_t *t)
{
	int i, min_i = -1;
	long k, min = LONG_MAX;
...
void fp_for(int n_threads, void (*func)(void*,long,int), void *data, long m, long n)
{
	if (n_threads > 1) {
		...
		fp_for_t t;
		pthread_t *tid;
		t.func = func, t.data = data, t.n_threads = n_threads, t.m = m, t.n = n;

		t.w = (ktf_worker_t*)calloc(n_threads, sizeof(ktf_worker_t));
		tid = (pthread_t*)calloc(n_threads, sizeof(pthread_t));
		for (i = 0; i < n_threads; ++i)
			t.w[i].t = &t, t.w[i].i = m + i;
		for (i = 0; i < n_threads; ++i) pthread_create(&tid[i], 0, ktf_worker, &t.w[i]);
		for (i = 0; i < n_threads; ++i) pthread_join(tid[i], 0);
		free(tid); free(t.w);
	} else {
		long j;
		for (j = m; j < n; ++j) func(data, j, 0);
	}
}
